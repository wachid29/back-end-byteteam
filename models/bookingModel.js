...
const findbyId = (id_booking) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT * FROM bookings WHERE id_booking=$1`,
      [id_booking],
...
const findId_user = (id_user) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT * FROM bookings WHERE id_user=$1 ORDER BY id_booking DESC`,
      [id_user],
      (error, result) => { if (error) { reject(error) } else { resolve(result) } }
    );
  });
...
const findId_ticket = (id_ticket) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT * FROM newticket WHERE id_ticket=$1`,
      [id_ticket],
      (error, result) => { if (error) { reject(error) } else { resolve(result) } }
    );
  });
...
const showAllByIdBookingIdUser = (id_booking, id_user) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT * FROM bookings WHERE id_booking=$1 AND id_user=$2 ORDER BY id_booking DESC`,
      [id_booking, id_user],
      (error, result) => { if (error) { reject(error) } else { resolve(result) } }
    );
  });
...
const statuspayment = (status_payment, id_booking) => {
  return new Promise((resolve, reject) => {
    db.query(`UPDATE bookings SET status_payment=$1 WHERE id_booking=$2`,
...
const findbyIdAndStatus = (id_booking, status_payment) => {
  return new Promise((resolve, reject) => {
    db.query(`SELECT * FROM bookings WHERE id_booking=$1 AND status_payment=$2`,
